...
namespace Clinica_Frba.Listados_Estadisticos
{
    public partial class frmListadosEstadisticos : Form
    {
        ...
        private void btn_top1_Click(object sender, EventArgs e)
        {
            ...
            est.consulta = "SELECT TOP 5 ROW_NUMBER() OVER (ORDER BY COUNT(a.cam_id) DESC,esp_nombre_especialidad) as Puesto,esp_nombre_especialidad as Especialidad,Count(a.cam_id) as Cantidad FROM SIGKILL.especialidad LEFT JOIN (SELECT * 	FROM SIGKILL.cancelacion_atencion_medica,SIGKILL.turno,SIGKILL.agenda_profesional WHERE cam_turno_id=trn_id AND trn_agenda=agp_id AND cam_tipo_cancelacion in (2,3,4,5) AND MONTH(cam_fecha_cancelacion) in ({0}) AND YEAR(cam_fecha_cancelacion) ={1}) as a ON (a.agp_especialidad=esp_id) WHERE esp_id>0 GROUP BY esp_nombre_especialidad ORDER BY 3 DESC,2";

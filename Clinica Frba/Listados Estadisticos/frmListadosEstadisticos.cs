...
namespace Clinica_Frba.Listados_Estadisticos
{
    public partial class frmListadosEstadisticos : Form
    {
        ...
        private void btn_top4_Click(object sender, EventArgs e)
        {
            var est = new Estadistica();
            est.name = ((Button)sender).Text;
            est.consulta = "SELECT TOP 10 Nombre,Apellido,SUM(Cantidad) as Cantidad FROM ((SELECT trn_afiliado,afil_nombre as Nombre,afil_apellido as Apellido,COUNT(*) as Cantidad 	FROM SIGKILL.bono_consulta,SIGKILL.consulta,SIGKILL.turno,SIGKILL.afiliado 	WHERE bonoc_consumido=1 AND cons_bono_consulta=bonoc_id AND cons_turno=trn_id AND trn_afiliado=afil_numero AND bonoc_afiliado!=trn_afiliado AND MONTH(trn_fecha_hora) in ({0}) AND YEAR(trn_fecha_hora)={1} 	GROUP BY trn_afiliado,afil_nombre,afil_apellido) 	Union 	(SELECT trn_afiliado,(SELECT afil_nombre FROM SIGKILL.afiliado WHERE afil_numero=trn_afiliado) as Nombre,(SELECT afil_apellido FROM SIGKILL.afiliado WHERE afil_numero=trn_afiliado) as Apellido, COUNT(*) as Cantidad 	FROM SIGKILL.bono_farmacia,SIGKILL.consulta,SIGKILL.turno	WHERE bonof_consumido=1 AND cons_id=bonof_consulta AND cons_turno=trn_id  AND bonof_afiliado!=trn_afiliado AND MONTH(trn_fecha_hora) in ({0}) AND YEAR(trn_fecha_hora)={1} 	GROUP BY trn_afiliado)) as a GROUP BY a.trn_afiliado,a.Nombre,a.Apellido ORDER BY 3 DESC,2 DESC,1 DESC";
            est.semestre = combo_semestre.SelectedIndex + 1;
            new Clinica_Frba.Listados_Estadisticos.frmResultadosEstadisticos(est).Show();
        }

...
namespace Clinica_Frba.ClasesDatosTablas
{
    public class Profesional
    {
        ...
        {            
            ...
            "(SELECT trn_profesional,trn_afiliado,{1},'{2}',trn_fecha_hora,'{3}' FROM SIGKILL.turno WHERE  trn_fecha_hora > '{0}' AND trn_profesional = {4} AND trn_id not in (SELECT cons_turno FROM SIGKILL.consulta))", fin.ToString("yyyy-MM-dd"), "7", "Baja de Profesional", Properties.Settings.Default.Date.ToString("yyyy-MM-dd"),this.pro_id);
            runner.Delete("DELETE FROM SIGKILL.turno WHERE trn_fecha_hora > '{0}' AND trn_profesional = {1} AND trn_id not in (SELECT cons_turno FROM SIGKILL.consulta)", fin.ToString("yyyy-MM-dd"),pro_id);
        }

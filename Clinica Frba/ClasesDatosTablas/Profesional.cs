...
namespace Clinica_Frba.ClasesDatosTablas
{
    public class Profesional
    {
        ...
        {            
            ...
            "(SELECT trn_profesional,trn_afiliado,{1},'{2}',trn_fecha_hora,'{3}',trn_id FROM SIGKILL.turno WHERE  trn_fecha_hora > '{0}' AND trn_profesional = {4} AND trn_id not in (SELECT cons_turno FROM SIGKILL.consulta) AND trn_valido=1)", fin.ToString("yyyy-MM-dd"), "7", "Baja de Profesional", Properties.Settings.Default.Date.ToString("yyyy-MM-dd"), this.pro_id);
            runner.Delete("UPDATE SIGKILL.turno SET trn_valido=0 WHERE trn_fecha_hora > '{0}' AND trn_profesional = {1} AND trn_id not in (SELECT cons_turno FROM SIGKILL.consulta) AND trn_valido=1", fin.ToString("yyyy-MM-dd"), pro_id);
        }
